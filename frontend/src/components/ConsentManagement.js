...
const ConsentManagement = ({ account }) => {
  ...
  useEffect(() => {
    const fetchConsents = async () => {
      ...
      try {
        ...
        const data = await apiService.getConsents(null, statusFilter);
  ...
  const handleCreateConsent = async (e) => {
    ...
    try {
      ...
      const newConsent = await apiService.createConsent(consentPayload);
      alert("Consent created successfully with ID: " + newConsent.id);
